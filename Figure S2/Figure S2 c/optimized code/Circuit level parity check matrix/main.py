...
L = 17 # width of the lattice of data qubits
distance = [5,9,12,16,22][H-4] # distance of the code

nbar = 11 # number of photons
k1surk2 = np.logspace(-5,np.log10(3e-3),20)[int(sys.argv[2])] # single photon loss rate/two-photon dissipation rate
...
print("number of space time syndromes ", (H-2)*L*(distance+1) )
print("number of error mechanisms ",nb_pauli_errors)

############## Calculate the circuit level parity check matrix ##############

# store the triggered syndromes by every error mechanism
CircuitLevelMatrix = np.zeros(((H-2)*L*(distance+1),nb_pauli_errors))
...
for i in range(fractalcode.circuit.nbcol):
    ...
    for j in range(column.nbgates):
        ...
        for k in range(possible_error):
                ...
                CircuitLevelMatrix[:(H-2)*L*distance,err_counter] = fractalcode.circuit.Mmt
                ...
                for l in range(len(fractalcode.CheckMatrix)):
                    ...
                    CircuitLevelMatrix[(H-2)*L*distance + l,err_counter] = syndrome%2
                 
	            ## Compute error probability
                if gate.name == "CX":
                    if gate.error == 1:
                        ProbaMatrix[err_counter] = nbar*k1surk2 + np.pi**2/64/nbar
                    if gate.error == 2:
                        ProbaMatrix[err_counter] = 1/2*nbar*k1surk2
                    if gate.error == 3:
                        ProbaMatrix[err_counter] = 1/2*nbar*k1surk2
                if gate.name == "I":
                    ProbaMatrix[err_counter] = nbar*k1surk2
                if gate.name == "MX":
                    ProbaMatrix[err_counter] = nbar*k1surk2
                if gate.name == "P":
                    ProbaMatrix[err_counter] = nbar*k1surk2
...
newCircuitLevelMatrix = np.zeros(((H-2)*L*(distance+1),nb_pauli_errors))

for l in range(len(CircuitLevelMatrix[0])):
    for n in range(L*(H-2)):
...
for l in range(len(CircuitLevelMatrix[0])):
    for m in range(1,distance+1):
        for n in range(L*(H-2)):
            newCircuitLevelMatrix[m*L*(H-2) + n,l] = (CircuitLevelMatrix[m*L*(H-2) + n,l] 
                                                + CircuitLevelMatrix[(m-1)*L*(H-2) + n,l])%2
...
CircuitLevelMatrix_final = StackMatrix_final[:(H-2)*L*(distance+1),:]
ErrorMatrix_final = StackMatrix_final[(H-2)*L*(distance+1):,:]

print("number of columns after factorizing ",len(CircuitLevelMatrix_final[0]))
