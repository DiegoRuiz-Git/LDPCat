...
FinalError = np.load("../Circuit level parity check matrix/ParityCheck/ErrorMatrix" + sys.argv[1] + '_' + sys.argv[2] + ".npy")
...
Tshape_code = css_code(hx = H,hz = np.zeros((1,len(H[0])),dtype = int)) 

# initialize the decoder
bpd=bposd_decoder(
    Tshape_code.hx, #the parity check matrix
...
def MonteCarlo():
    ...
    while error_count < 2:
        ...
        # compute syndromes
        syndrome = Tshape_code.hx@error.T %2
