...
p_prep = p # preparation error
p_meas = p # measurement error
p_CX_Z1 = p/3 # CNOT ancilla error
p_CX_Z2 = p/3 # CNOT target error
p_CX_Z1Z2 = p/3 # CNOT correlated error
p_prep_w = p # Idle error during preparation
p_meas_w = p # Idle error during measurement
p_CX_w = p # Idle error during CNOT

# probabilities associated to edges of the error graph
# horizontal edge (measurement errors)
p_hor = combine([p_prep,p_CX_Z1,p_CX_Z1,p_meas])
...
p_in_bottom = combine([p_prep_w,p_CX_w,p_CX_Z1Z2])
# vertical edge on the first row
p_top = combine([p_CX_Z2,p_CX_w,p_meas_w,p_prep_w,p_CX_Z1Z2])
# vertical edge on the last row
p_bottom = combine([p_CX_Z2,p_meas_w,p_prep_w,p_CX_w,p_CX_Z1Z2])
# output vertical edge
p_out = combine([p_CX_Z2,p_meas_w])
# top output vertical edge
p_out_top = combine([p_CX_Z2,p_CX_w,p_meas_w])
# diagonal edge
p_diag = combine([p_CX_Z2,p_CX_Z1Z2])
