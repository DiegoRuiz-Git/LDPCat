...
// Calculate the distance of the code with SAT solver
int CalculateDistanceSAT(bool G[MAX][MAX],int k,int n);
...
int CyclicCode(int l, int m, int b,vector<tuple<vector<vector<int>>,int,int,float>>& result,
	...
	if (k>0){
		...
		// if more than 34 logical qubits, use SAT solver
		if (k>34)
		    d = CalculateDistanceSAT(G,k,n);
		// if less, brute force
		else
...
void CreateStabilisateur(bool stabilizer[MAX][MAX], int b){
	...
	// Initialize stabilizer with zeros
	for (int i = 0; i < MAX; i++) {
		for (int j = 0; j < MAX; j++)
			stabilizer[i][j] = 0;
	}
...
int CalculateDistanceSAT(bool G[MAX][MAX],int k,int n){
	...
	// Make a guess of the distance just by looking 
	// at the Hamming weight of the rows of G
    int init_distance = n;
    for (int i = 0; i < k; i++){
        int HammingWeight = 0;
        for (int j = 0; j < n; j++)
            HammingWeight += G[i][j];
        if (HammingWeight < init_distance)
            init_distance = HammingWeight;
    }

    int d = init_distance;
    int dmax = init_distance;
    int dmin = 1;

	// Find the distance by dichotomy
    while (dmin != dmax){
        ...
        // define the SAT variables
		// they correspond to the decomposition of the codeword
		// in the basis of G rows
        z3::expr_vector Weight(c);
        for (int i = 0; i < k; i++){
            std::stringstream x_name;
            x_name << "b" << i;
            Weight.push_back(c.bool_const(x_name.str().c_str()));
        }

        // At least one weight must be 1
        s.add(atleast(Weight,1));

		// Initialize codeword
        std::vector<z3::expr> Codeword;
        for (int i = 0; i < n; i++){
            z3::expr f = c.bool_val(false);
            Codeword.push_back(f);
        }
            
		// Calculate codeword from SAT variables and G
        for (int i = 0; i < k; i++) {
            for (int j = 0; j < n; j++) {
                if (G[i][j] == 1)
                    Codeword[j] = Codeword[j] ^ (Weight[i] && c.bool_val(true));
                else
                    Codeword[j] = Codeword[j] ^ (Weight[i] && c.bool_val(false));
            }
        }

		// convert Codeword to expr_vector
        z3::expr_vector CodewordExprVec(c);
        for (auto &e : Codeword)
            CodewordExprVec.push_back(e);

        // Call SAT solver
		// Can a codeword with a Hamming Weight of d be found ?
        s.add(atmost(CodewordExprVec,d));
